...
class Activity_MessageAdapter(context: Context?, messages: ArrayList<Activity_Message>) :
    ArrayAdapter<Activity_Message?>(context!!, 0, messages!! as List<Activity_Message?>) {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val message: Activity_Message? = getItem(position)
        // Check if an existing view is being reused, otherwise inflat the view
        if (convertView == null) {
            convertView =
                LayoutInflater.from(context).inflate(R.layout.shopper_activity_message_adapter, parent, false)
        }
